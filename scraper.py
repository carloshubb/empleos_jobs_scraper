...
class CostaRicaJobsScraper:
    ...
    def check_if_more_pages(self, html):
        ...
        pagination = soup.find_all('a', href=re.compile(r'Pag=\d+', re.IGNORECASE))
        if pagination:
            return True
    ...
    def extract_category(self, soup):
        ...
        if category_section:
            return category_section.get_text(strip=True)
    ...
    def scrape_all_pages(self, max_pages=44):
        ...
        while page <= max_pages:
            ...
            print(f"Found {len(job_urls)} unique job URLs on page {page}")
            
            if not job_urls:
                print("No job URLs found on this page, stopping...")
                break
...
if __name__ == "__main__":
    # For initial scrape (all pages up to 44):
    initial_scrape()
    
    # For weekly updates (first page only) - comment out initial_scrape and uncomment this:
    # weekly_update()